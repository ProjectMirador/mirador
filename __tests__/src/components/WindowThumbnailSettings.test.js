...
describe('WindowThumbnailSettings', () => {
  it('renders all elements correctly', () => {
    ...
    expect(
      screen.getByRole('menuitemradio', { name: /Off/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('menuitemradio', { name: /Bottom/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('menuitemradio', { name: /Right/ }),
    ).toBeInTheDocument();
  });
  it('for far-bottom it should set the correct label active (by setting the secondary color)', () => {
    ...
    expect(
      screen.getByRole('menuitemradio', { name: /Off/ }).querySelector('svg'),
    ).not.toHaveClass('MuiSvgIcon-colorSecondary');
  });
  ...
  it('updates state when the thumbnail config selection changes', async () => {
    ...
    expect(setWindowThumbnailPosition).toHaveBeenCalledWith(
      'xyz',
      'far-bottom',
    );
