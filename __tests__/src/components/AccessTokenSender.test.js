...
function createWrapper(props) {
  return render(
    <AccessTokenSender handleAccessTokenMessage={() => {}} {...props} />,
  );
}
...
describe('AccessTokenSender', () => {
  ...
  it('renders properly', () => {
    ...
    expect(container.querySelector('iframe')).toHaveAttribute(
      'src',
      'http://example.com/?origin=http%3A%2F%2Flocalhost&messageId=http%3A%2F%2Fexample.com',
    );
  });
  ...
  it('triggers an action when the iframe sends a message', () => {
    ...
    window.dispatchEvent(
      new MessageEvent('message', {
        data: { messageId: 'http://example.com' },
      }),
    );
    expect(handleAccessTokenMessage).toHaveBeenCalledWith({
      messageId: 'http://example.com',
    });
  });
  ...
  it('ignores iframe messages with the wrong messageId', () => {
    ...
    window.dispatchEvent(
      new MessageEvent('message', {
        data: { messageId: 'http://example.com/123' },
      }),
    );
