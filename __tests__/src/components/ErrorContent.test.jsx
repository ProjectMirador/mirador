...
describe('ErrorContent', () => {
  it('should render everything when showJsError is true', async () => {
    render(<ErrorContent error={new Error('Invalid JSON')} windowId="xyz" manifestId="foo" classes={{}} />, {
      preloadedState: {
        config: {
          window: {
            showJsError: true,
  ...
  it('does not render the alert when showJsError is false ', async () => {
    render(<ErrorContent error={new Error('Invalid JSON')} windowId="xyz" manifestId="foo" showJsError={false} classes={{}} />, {
      preloadedState: {
        config: {
          window: {
            showJsError: false,
