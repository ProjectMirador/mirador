...
describe('ZoomControls', () => {
  ...
  describe('with getZoomBounds', () => {
    ...
    it('clamps the requested zoom-in to the live max, even if redux is stale', async () => {
      updateViewport = vi.fn();
      createWrapper({
        getZoomBounds: () => ({ maxZoom: 0.5, minZoom: 0.1, zoom: 0.4 }),
        updateViewport,
    ...
    it('clamps the requested zoom-out to the live min, even if redux is stale', async () => {
      updateViewport = vi.fn();
      createWrapper({
        getZoomBounds: () => ({ maxZoom: 0.5, minZoom: 0.1, zoom: 0.15 }),
        updateViewport,
    ...
    it('disables zoom-in once the live zoom has reached the max', () => {
      createWrapper({ getZoomBounds: () => ({ maxZoom: 0.5, minZoom: 0.1, zoom: 0.5 }) });
    ...
    it('disables zoom-out once the live zoom has reached the min', () => {
      createWrapper({ getZoomBounds: () => ({ maxZoom: 0.5, minZoom: 0.1, zoom: 0.1 }) });
    ...
    it('does not disable either button while strictly between min and max', () => {
      createWrapper({ getZoomBounds: () => ({ maxZoom: 0.5, minZoom: 0.1, zoom: 0.3 }) });
