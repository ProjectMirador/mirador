...
describe('SidebarIndexList', () => {
  ...
  it('renders all needed elements for the thumbnail view', () => {
    ...
    expect(
      screen.getByRole('menuitem', { name: 'Test 19 Canvas: 1' }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('menuitem', { name: 'Image 1' }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('menuitem', { name: 'Image 2' }),
    ).toBeInTheDocument();

    expect(container.querySelectorAll('img').length).toBe(3);

    expect(screen.getByRole('menuitem', { name: 'Image 1' })).toHaveClass(
      'Mui-selected',
    );
  });
