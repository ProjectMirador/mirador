...
describe('selectCurrentAuthServices', () => {
  ...
  it('returns the next auth service to try', () => {
    expect(selectCurrentAuthServices(state, { windowId: 'w' })[0].id).toEqual(
      'external',
    );
  });
  ...
  it('returns the service if the next auth service is interactive', () => {
    ...
    expect(
      selectCurrentAuthServices({ ...state, auth }, { windowId: 'w' })[0].id,
    ).toEqual('login');
  });
  ...
  it('returns the last attempted auth service if all of them have been tried', () => {
    ...
    expect(
      selectCurrentAuthServices({ ...state, auth }, { windowId: 'x' })[0].id,
    ).toEqual('external');
    expect(
      selectCurrentAuthServices({ ...state, auth }, { windowId: 'y' })[0],
    ).toBeUndefined();
  });
