...
describe('AnnotationResource', () => {
  describe('id', () => {
    ...
    it('handles the case where an id is only present in the resource', () => {
      expect(
        new AnnotationResource({ resource: [{ '@id': 'bar' }] }).id,
      ).toEqual('bar');
    });
  });
  ...
  describe('isOnlyTag', () => {
    it('when the only motivation is tagging', () => {
      expect(
        new AnnotationResource({ motivation: 'oa:tagging' }).isOnlyTag(),
      ).toBe(true);
    });
  ...
  describe('targetId', () => {
    it('removes fragmentSelector coords from string targets', () => {
      expect(
        new AnnotationResource({ on: 'www.example.com/#xywh=10,10,100,200' })
          .targetId,
    ...
    it('can target an array of selectors', () => {
      expect(
        new AnnotationResource({
          on: [{ full: 'www.example.com/#xywh=10,10,100,200' }],
        }).targetId,
      ).toEqual('www.example.com/');
    });
  ...
  describe('motivations', () => {
    ...
    it('with a single motivation', () => {
      expect(
        new AnnotationResource({ motivation: 'oa:commenting' }).motivations,
      ).toEqual(['oa:commenting']);
    });
    it('with multiple motivations', () => {
      expect(
        new AnnotationResource({ motivation: ['oa:commenting', 'sc:funstuff'] })
          .motivations,
  ...
  describe('resources', () => {
    ...
    it('with a single resource', () => {
      expect(new AnnotationResource({ resource: 'foo' }).resources).toEqual([
        'foo',
      ]);
    });
    it('with multiple resources', () => {
      expect(
        new AnnotationResource({ resource: ['foo', 'bar'] }).resources,
      ).toEqual(['foo', 'bar']);
    });
  });
  describe('on', () => {
    ...
    it('with multiple on', () => {
      expect(new AnnotationResource({ on: ['foo', 'bar'] }).on).toEqual([
        'foo',
        'bar',
      ]);
    });
  ...
  describe('chars', () => {
    ...
    it('with a single resource', () => {
      expect(
        new AnnotationResource({ resource: { chars: 'foo' } }).chars,
      ).toEqual('foo');
    });
    it('with multiple resources', () => {
      expect(
        new AnnotationResource({
          resource: [{ chars: 'foo' }, { chars: 'bar' }],
        }).chars,
      ).toEqual('foo bar');
    });
  });
  describe('fragmentSelector', () => {
    ...
    it('url without a fragment', () => {
      expect(
        new AnnotationResource({
          on: { selector: { value: 'www.example.com' } },
        }).fragmentSelector,
      ).toEqual(null);
    });
  });
  describe('svgSelector', () => {
    ...
    it('without specified type', () => {
      expect(
        new AnnotationResource({ on: [{ selector: { item: {} } }] })
          .svgSelector,
      ).toEqual(null);
    });
  });
