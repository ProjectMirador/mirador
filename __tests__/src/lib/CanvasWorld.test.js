...
const canvases = Utils.parseManifest(fixture)
  .getSequences()[0]
  .getCanvases()
  .map(wrapCanvas);
...
describe('CanvasWorld', () => {
  describe('constructor', () => {
    it('sets canvases', () => {
      expect(
        new CanvasWorld([wrapCanvas(1)]).canvases.map((c) => c.canvas),
      ).toEqual([1]);
    });
  });
  ...
  describe('worldBounds', () => {
    it('calculates a world bounds for the given canvases', () => {
      expect(new CanvasWorld([canvases[1]]).worldBounds()).toEqual([
        0, 0, 6501, 4421,
      ]);
      expect(new CanvasWorld(canvasSubset).worldBounds()).toEqual([
        0, 0, 9153, 4288,
      ]);
    });
  });
  describe('contentResourceToWorldCoordinates', () => {
    ...
    it('supports RTL orientations', () => {
      expect(
        new CanvasWorld(
          ...
          null,
          'right-to-left',
        ).contentResourceToWorldCoordinates({
          id: 'https://stacks.stanford.edu/image/iiif/rz176rt6531%2FPC0170_s3_Tree_Calendar_20081101_152516_0410/full/full/0/default.jpg',
        }),
    ...
    it('when placed by a fragment contains the offset', () => {
      const subject = new CanvasWorld(
        [
          Utils.parseManifest(fragmentFixture)
            .getSequences()[0]
            .getCanvases()[0],
        ].map(wrapCanvas),
      );
  ...
  describe('layerOpacityOfImageResource', () => {
    ...
    it('returns 0 if the layer is currently hidden', () => {
      ...
      expect(
        new CanvasWorld(canvases, layers).layerOpacityOfImageResource(
          tileSource1,
        ),
      ).toEqual(0);
    });
  ...
  describe('layerIndexOfImageResource', () => {
    ...
    it('returns the inverse of the configured index', () => {
      ...
      expect(
        new CanvasWorld(canvases, layers).layerIndexOfImageResource(
          tileSource0,
        ),
      ).toEqual(0);
    });
  ...
  describe('canvasAtPoint', () => {
    it('returns the canvas  at a particular point', () => {
      ...
      expect(canvasWorld.canvasAtPoint({ x: 1, y: 1 }).id).toEqual(
        canvases[1].id,
      );
      expect(canvasWorld.canvasAtPoint({ x: 6400, y: 1 }).id).toEqual(
        canvases[2].id,
      );
    });
