import { ManifestResource, Resource, Service, Utils } from 'manifesto.js';
import getThumbnail, {
  ThumbnailFactory,
} from '../../../src/lib/ThumbnailFactory';
...
describe('getThumbnail', () => {
  ...
  const iiifLevel1Service = iiifService(
    url,
    { height: 2000, width: 1000 },
    { profile: 'level1' },
  );
  const iiifLevel2Service = iiifService(
    url,
    { height: 2000, width: 1000 },
    { profile: 'level2' },
  );
    ...
    for (const type of ['Collection', 'Manifest', 'Canvas', 'Image']) {
      describe('with a thumbnail', () => {
        ...
        it('return the IIIF service of the thumbnail', () => {
          expect(
            createSubject(
              { '@id': 'xyz', '@type': type, thumbnail: iiifLevel1Service },
              type,
            ),
        ...
        describe('with image size constraints', () => {
          it('does nothing with a static resource', () => {
            expect(
              createSubject(
                { '@id': 'xyz', '@type': type, thumbnail: { '@id': url } },
                type,
                { maxWidth: 50 },
              ),
            ).toMatchObject({ url });
          ...
          it('does nothing with a IIIF level 0 service', () => {
            expect(
              createSubject(
                { '@id': 'xyz', '@type': type, thumbnail: iiifLevel0Service },
                type,
                { maxWidth: 50 },
              ),
            ).toMatchObject({ url: 'arbitrary-url' });
          });
          ...
          it('calculates constraints for a IIIF level 1 service', () => {
            expect(
              ...
              height: 300,
              url: `${url}/full/150,/0/default.jpg`,
              width: 150,
            });
          });
          ...
          it('calculates constraints for a IIIF level 2 service', () => {
            expect(
              ...
              height: 200,
              url: `${url}/full/!150,200/0/default.jpg`,
              width: 100,
            });
          });
  ...
  describe('with an image resource', () => {
    ...
    describe('with a level 0 IIIF service', () => {
      ...
      it('uses embedded sizes to find an appropriate size', () => {
        ...
        expect(
          createImageSubject(obj, { maxHeight: 120, maxWidth: 120 }),
  ...
  describe('with a canvas', () => {
    it('uses the thumbnail', () => {
      expect(
        createSubject(
          { ...canvas.__jsonld, thumbnail: { ...iiifLevel1Service } },
          'Canvas',
        ),
      ).toMatchObject({ url: `${url}/full/,120/0/default.jpg` });
    });
    ...
    it('uses the first image resource', () => {
      expect(getThumbnail(canvas)).toMatchObject({
        url: 'https://stacks.stanford.edu/image/iiif/hg676jb4964%2F0380_796-44/full/,120/0/default.jpg',
      });
    });
    ...
    it('uses embedded sizes of a IIIF Image API service to find an appropriate size', () => {
      ...
      expect(
        createSubject(myCanvas, 'Canvas', { maxHeight: 120, maxWidth: 120 }),
      ).toMatchObject({
        height: 125,
        url: `${url}/full/125,125/0/default.jpg`,
        width: 125,
      });
    });
  });
  ...
  describe('with a manifest', () => {
    it('does nothing with a plain URL', () => {
      ...
      expect(
        getThumbnail(manifestWithThumbnail, { maxWidth: 50 }),
      ).toMatchObject({ url });
    });
    ...
    it('uses the thumbnail', () => {
      ...
      expect(getThumbnail(manifestWithThumbnail)).toMatchObject({
        url: `${url}/full/,120/0/default.jpg`,
      });
    });

    it('uses the startCanvas', () => {
      const manifestWithStartCanvas = Utils.parseManifest({
        ...manifest.__jsonld,
        start: {
          id: 'https://purl.stanford.edu/fr426cg9537/iiif/canvas/fr426cg9537_1',
        },
      });
      expect(getThumbnail(manifestWithStartCanvas)).toMatchObject({
        url: 'https://stacks.stanford.edu/image/iiif/fr426cg9537%2FSC1094_s3_b14_f17_Cats_1976_0005/full/,120/0/default.jpg',
      });
    });

    it('uses the first canvas', () => {
      expect(getThumbnail(manifest)).toMatchObject({
        url: 'https://stacks.stanford.edu/image/iiif/hg676jb4964%2F0380_796-44/full/,120/0/default.jpg',
      });
    });
  });
  ...
  describe('with a collection', () => {
    it('uses the thumbnail', () => {
      ...
      expect(getThumbnail(collection)).toMatchObject({
        url: `${url}/full/,120/0/default.jpg`,
      });
    });
    ...
    it('uses the first manifest', () => {
      ...
      expect(getThumbnail(collection)).toMatchObject({
        url: 'https://example.org/manifest1/thumbnail.jpg',
      });
    });
  });
});
...
describe('picking the best format', () => {
  ...
  it('can be filtered by application preferred formats', () => {
    ...
    expect(
      createSubject(myCanvas, 'Canvas', { preferredFormats: ['png', 'jpg'] }),
...
describe('selectBestImageSize', () => {
  ...
  it('selects the smallest size larger than the target, if one is available', () => {
    ...
    expect(
      ThumbnailFactory.selectBestImageSize(service, targetWidth * targetHeight),
    ).toEqual(sizes[1]);
  });
