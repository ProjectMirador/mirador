...
describe('getThumbnail', () => {
      ...
      describe('with a thumbnail', () => {
        ...
        describe('with image size constraints', () => {
          it('does nothing with a static resource', () => {
            expect(
              createSubject({ '@id': 'xyz', '@type': type, thumbnail: { '@id': url } }, type, {
                maxWidth: 50,
              }),
            ).toMatchObject({ url });
          });
          ...
          it('does nothing with a IIIF level 0 service', () => {
            expect(
              createSubject({ '@id': 'xyz', '@type': type, thumbnail: iiifLevel0Service }, type, {
                maxWidth: 50,
              }),
          ...
          it('calculates constraints for a IIIF level 1 service', () => {
            ...
            ).toMatchObject({ height: 300, url: `${url}/full/150,/0/default.jpg`, width: 150 });
          });

          it('calculates constraints for a IIIF level 2 service', () => {
            expect(
              createSubject({ '@id': 'xyz', '@type': type, thumbnail: iiifLevel2Service }, type, {
                maxHeight: 200,
                maxWidth: 150,
              }),
            ).toMatchObject({ height: 200, url: `${url}/full/!150,200/0/default.jpg`, width: 100 });
          });
  ...
  describe('with an image resource', () => {
    ...
    describe('with a level 0 IIIF service', () => {
      ...
      it('uses embedded sizes to find an appropriate size', () => {
        ...
        expect(createImageSubject(obj, { maxHeight: 120, maxWidth: 120 })).toMatchObject({
          height: 125,
          width: 125,
        });
      });
  ...
  describe('with a canvas', () => {
    ...
    it('uses embedded sizes of a IIIF Image API service to find an appropriate size', () => {
      ...
      expect(createSubject(myCanvas, 'Canvas', { maxHeight: 120, maxWidth: 120 })).toMatchObject({
        height: 125,
