...
export function* setWindowStartingCanvas(action) {
  ...
  if (canvasId) {
    const thunk = yield call(setCanvas, windowId, canvasId, null, {
      preserveViewport: !!action.payload,
    });
    yield put(thunk);
  } else {
    const getMiradorManifest = yield select(getMiradorManifestWrapper);
    const manifestoInstance = yield select(getManifestoInstance, {
      manifestId,
    });
    if (manifestoInstance) {
      // set the startCanvas
...
export function* setWindowDefaultSearchQuery(action) {
  ...
  if (searchService && companionWindowId) {
    const searchId =
      searchService && `${searchService.id}?q=${defaultSearchQuery}`;
    yield put(
      fetchSearch(windowId, companionWindowId, searchId, defaultSearchQuery),
    );
  }
}

/** @private */
export function getAnnotationsBySearch(
  state,
  { canvasIds, companionWindowIds, windowId },
) {
  const annotationBySearch = companionWindowIds.reduce(
    (accumulator, companionWindowId) => {
      ...
      const hitAnnotation = resourceAnnotations.find((r) =>
        canvasIds.includes(r.targetId),
      );
...
export function* setCurrentAnnotationsOnCurrentCanvas({
  ...
  if (Object.values(annotationBySearch).length > 0) {
    ...
    yield put(
      selectAnnotation(windowId, Object.values(annotationBySearch)[0][0]),
    );
  }
}
...
export function* updateVisibleCanvases({ windowId }) {
  ...
  const visibleCanvases = yield select(getCanvasGrouping, {
    canvasId,
    windowId,
  });
  yield put(
...
export function* fetchInfoResponses({
  visibleCanvases: visibleCanvasIds,
  windowId,
}) {
  const canvases = yield select(getCanvases, { windowId });
  const infoResponses = yield select(selectInfoResponses);
  const visibleCanvases = (canvases || []).filter((c) =>
    visibleCanvasIds.includes(c.id),
  );
