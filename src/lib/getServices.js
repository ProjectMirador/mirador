...
export function anyAuthServices(resource) {
  return (
    resource &&
    Utils.getServices(resource).filter(
      (s) =>
        (s.getProfile() && s.getProfile().match(/http:\/\/iiif.io\/api\/auth\//)) || // Auth 1
        (s.getProperty('type') && s.getProperty('type').match(/^Auth.*2$/)),
    )
  ); // Auth 2
}
...
export function anyProbeServices(resource) {
  return (
    resource &&
    Utils.getServices(resource).filter(
      (s) => s.getProfile() === 'http://iiif.io/api/auth/1/probe' || s.getProperty('type') === 'AuthProbeService2',
    )
  );
}
...
export function getProbeService(resource) {
  return resource && anyProbeServices(resource)[0];
}
