...
export default class MiradorCanvas {
  ...
  get imageResources() {
    ...
    const resources = flattenDeep([
      ...
      this.canvas
        .getContent()
        .map((i) =>
          i.__jsonld.body.type === 'Choice' ? i.__jsonld.body : i.getBody(),
        ),
    ]);
  ...
  get textResources() {
    ...
    return flatten(
      resources.filter((resource) => resource.getProperty('type') === 'Text'),
    );
  }
  ...
  get videoResources() {
    ...
    return flatten(
      resources.filter((resource) => resource.getProperty('type') === 'Video'),
    );
  }
  ...
  get audioResources() {
    ...
    return flatten(
      resources.filter((resource) => resource.getProperty('type') === 'Sound'),
    );
  }
  ...
  get v2VttContent() {
    ...
    return flatten(
      resources.filter(
        (resource) => resource.getProperty('format') === 'text/vtt',
      ),
    );
  }
  ...
  get v3VttContent() {
    const resources = flattenDeep(
      this.canvasAnnotationPages.map((annoPage) => {
        const manifestoAnnoPage = new AnnotationPage(
          annoPage,
          this.canvas.options,
        );
